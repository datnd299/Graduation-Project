...
//                         Shifting integer value by 2 in left and replacing the two least significant digits with the message_byte_array values..
                        tmp = (byte) ((((integer_pixel_array[element] >> binary[channelIndex]) & 0xFF) & 0xFC) | ((messageEncodingStatus.getByteArrayMessage()[messageEncodingStatus.getCurrentMessageIndex()]
                                >> toShift[(shiftIndex++)
                                % toShift.length]) & 0x3));// 6
//                        int pVal = integer_pixel_array[element];
//                        int bVal =  binary[channelIndex];
//                        int pValShifted = pVal >> bVal;
//                        int pValAnded = pValShifted & 0xFF;
//                        int pValAnded2 = pValAnded & 0xFC;
//
//                        int currentMessageIndex = messageEncodingStatus.getCurrentMessageIndex();
//                        byte byteMessage = messageEncodingStatus.getByteArrayMessage()[currentMessageIndex];
//                        int toShiftVal = toShift[(shiftIndex++) % toShift.length];
//                        int byteMessageShifted = byteMessage>> toShiftVal;
//                        int byteMessageAnded = byteMessageShifted & 0x03;
//                        int tmp2 =  pValAnded2|byteMessageAnded;
//                        tmp = (byte) tmp2;
