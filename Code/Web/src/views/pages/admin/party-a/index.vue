    ...
    <template v-slot:item.com_acc="{ item }">
      <v-btn-toggle
       dense color="#033"
          ...
          <v-btn :loading="item.loading" v-if="item.status!=2" depressed  small color="green" @click="approvePartyA(item,2)" text-color="white">
            <i class="fas fa-check" style="color:white"></i>
            <!-- <v-icon style="color:red">fas fa-times</v-icon> -->
          </v-btn>
          <v-btn :loading="item.loading"  v-if="item.status!==0" depressed  small color="red" @click="approvePartyA(item,0)"  text-color="white">
...
import {getPartyAs,approvePartyA} from '@/api/admin/users'
export default {
  data() {
    return {
      isLoading:false,
